<script setup>
import { CircleCheck, Loader2, AlertCircle } from "lucide-vue-next";
...
<template>
  <article
      ...
      'flex flex-col dark:bg-black border-[8px] rounded-[32px] py-8 px-6',
      {
        ...
        'border-yellow-500': investmentStatus === 'pending',
        'border-colorPurpleCard': !isActive && investmentStatus !== 'pending',
        'opacity-50': disabled && !isActive,
      },
    ]"
  >
    <header class="relative">
      <div v-if="isActive || investmentStatus === 'pending'"
           :class="[
             'absolute -top-6 -right-4 py-1 px-3 text-white rounded-full text-sm flex items-center gap-2',
             isActive ? 'bg-green-500' : 'bg-yellow-500'
           ]">
        <CircleCheck v-if="isActive" class="w-4 h-4" />
        <AlertCircle v-else class="w-4 h-4" />
        {{ isActive ? 'Plan Activo' : 'Pendiente' }}
      </div>
      <span :class="[
    ...
    <button
        ...
        :disabled="disabled || isLoading || isActive || investmentStatus === 'pending'"
        :class="[
        'rounded-[8px] mt-7 w-full text-center py-3 text-[16px] font-semibold transition-all duration-200',
        {
          'bg-colorBgButton text-white hover:bg-opacity-90': !disabled && !isActive && !investmentStatus,
          'bg-gray-400 text-gray-200 cursor-not-allowed': disabled && !isActive,
          'bg-green-500 text-white cursor-default': isActive,
          'bg-yellow-500 text-white cursor-default': investmentStatus === 'pending'
        }
      ]"
    >
      ...
      <span>{{
          isActive ? 'Plan Actual' :
              investmentStatus === 'pending' ? 'En Revisión' :
                  'Elegir plan'
        }}</span>
    </button>
  </article>
